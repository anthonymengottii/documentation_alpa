---
title: "Autenticação"
description: "Aprenda como autenticar suas requisições na Alpa Gateway API"
---

## 🔐 Visão Geral

A Alpa Gateway API usa autenticação baseada em **API Keys** (Bearer Token) para garantir que apenas usuários autorizados possam acessar os recursos da API.

## 📋 Obtendo sua API Key

### Passo 1: Acesse o Dashboard

1. Faça login no [Dashboard Alpa](https://app.usealpa.com)
...
const response = await axios.get('https://alpa-sistema-api.onrender.com/api/payment-links', {
  headers: {
    'Authorization': `Bearer ${apiKey}`,
    'Content-Type': 'application/json'
  }
...
```env
ALPA_API_KEY=sua_api_key_aqui
ALPA_API_URL=https://alpa-sistema-api.onrender.com
```
...
```javascript
// Node.js
const apiKey = process.env.ALPA_API_KEY;

// Python
import os
api_key = os.getenv('ALPA_API_KEY')
```
...
```bash
curl --request POST \
  --url https://alpa-sistema-api.onrender.com/api/auth/google \
...
```bash
curl --request POST \
  --url https://alpa-sistema-api.onrender.com/api/auth/mfa/enable \
...
```bash
curl --request POST \
  --url https://alpa-sistema-api.onrender.com/api/auth/mfa/confirm \
  --header 'Authorization: Bearer SEU_JWT_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{ "code": "123456" }'
```
...
```bash
curl --request POST \
  --url https://alpa-sistema-api.onrender.com/api/auth/mfa/verify \
  --header 'Content-Type: application/json' \
