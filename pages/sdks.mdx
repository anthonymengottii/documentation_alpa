---
title: 'SDKs'
description: 'Bibliotecas oficiais para integração com a API da Alpa'
icon: 'boxes-stacked'
---

## O que são os SDKs da Alpa?

Os SDKs (Software Development Kits) da Alpa são bibliotecas oficiais que simplificam a comunicação com nossa API. Eles oferecem uma interface amigável e específica para cada linguagem, permitindo que você integre rapidamente PIX, cartão e boleto ao seu sistema.
...
console.log('Checkout:', alpa.paymentLinks.getCheckoutUrl(link.slug));
```
...
print('Checkout:', alpa.payment_links.get_checkout_url(link['slug']))
```
...
echo 'Checkout: ' . $alpa->paymentLinks->getCheckoutUrl($link['slug']);
```

```java Java
import com.alpa.AlpaClient;

var alpa = new AlpaClient(System.getenv("ALPA_API_KEY"));

var link = alpa.paymentLinks.create("Produto Premium", 9900, java.util.Map.of());
...
<AccordionGroup>
  ...
  <Accordion title="O que fazer se encontrar um bug em um SDK?">
    Abra uma issue no repositório GitHub do SDK específico. Nossa equipe está sempre atenta para corrigir bugs e melhorar as bibliotecas. Você também pode entrar em contato pelo e-mail <Icon icon="envelope" type="solid" /> suporte@usealpa.com.
  </Accordion>

  <Accordion title="Falta um SDK para a linguagem que eu uso. O que fazer?">
    Entre em contato pelo e-mail <Icon icon="envelope" type="solid" /> suporte@usealpa.com informando qual linguagem você precisa. Nossa equipe avaliará a demanda para incluir na roadmap.
  </Accordion>
</AccordionGroup>
...
<Card title="Precisa de ajuda com a integração?" icon="headset">
  Nossa equipe está disponível para ajudar. Entre em contato: <Icon icon="envelope" type="solid" /> suporte@usealpa.com
</Card>
